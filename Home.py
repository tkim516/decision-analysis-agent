...
Because the new laptop is under warranty for the first year, you will have no repair costs during that period. However, starting in the second year, you expect minimal maintenance of about $80 per year (for general upkeep or an extended warranty).
...
Immediate repairs (likely to be needed soon): $50 for a new battery and possibly a small hardware fix.
...
1) A clearly drawn decision tree with a decision node (buy or keep old laptop) and two chance nodes (one for repairs in Year 1, another for Year 2).
2) Labeled probabilities for the chance nodes and labeled cash inflows/outflows for all branches.
3) Calculated net values at the end of each possible branch outcome.
4) A short explanation of which option is better financially based on your expected values.
"""
...
from rag_evaluation_metrics import eval_faithfulness

retrieved_questions = [similar_question_text]
eval_faithfulness(target_question, retrieved_questions, agent_solution)
